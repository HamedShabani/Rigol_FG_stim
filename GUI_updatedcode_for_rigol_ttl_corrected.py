...
class RigolBurstGUI(tk.Tk):
    ...
    def sleep_until(self, target_time, start_time, total_time):
        """Sleeps until an absolute wall-clock time is reached, eliminating drift."""
        while time.time() < target_time:
            ...
            # Sleep in tiny 5ms chunks for high precision
            time.sleep(0.005)
    ...
    def run_sequence(self):
        ...
        def worker():
            try:
                ...
                # Reset start time right before loop starts so the progress bar is perfectly accurate
                ...
                for i in range(1, reps + 1):
                    ...
                    # Calculate the EXACT absolute clock times this trial should end
                    next_target += (dur + interval)
                    
                    # Sleep until the exact target time, absorbing the USB trigger delay
                    self.sleep_until(next_target, start_time, total_time)
