...
class KeysightAMGUI(tk.Tk):
    ...
    def build_ui(self):
        ...
        ttk.Label(main, text="Duration (s)").grid(row=3, column=0, sticky="w")
        ttk.Entry(main, textvariable=self.dur_var, width=12).grid(row=3, column=1, sticky="w")

        ttk.Label(main, text="Interval (s)").grid(row=3, column=2, sticky="w")
        ttk.Entry(main, textvariable=self.int_var, width=12).grid(row=3, column=3, sticky="w")

        ttk.Label(main, text="Reps").grid(row=4, column=0, sticky="w")
        ttk.Entry(main, textvariable=self.reps_var, width=12).grid(row=4, column=1, sticky="w")

        ops = ttk.Frame(main)
        ops.grid(row=5, column=0, columnspan=6, pady=(10, 0), sticky="we")
        ...
        prog = ttk.Frame(main)
        prog.grid(row=6, column=0, columnspan=6, sticky="we", pady=(10, 0))
    ...
    # ---------------- AM config ----------------
    def configure_am_base(self, carrier_freq, mod_freq, amp, depth):
        self.write("*CLS")
        time.sleep(0.1)
    ...
    def run_sequence(self):
        ...
        def worker():
            ...
            try:
                self.configure_am_base(carrier, mod, amp, depth)
                self.ui(
                    self.set_status,
                    f"Configured: Carrier={carrier} Hz | Mod={mod} Hz | Amp={amp} Vpp | Depth={depth}%"
                )
                self.show_last_error()
                ...
                for i in range(1, reps + 1):
                    ...
                    self.ui(self.set_status, f"AM cycle {i}/{reps}: ON")
                    self.am_on()
                    ...
                    while (time.time() - t0) < dur:
                        ...
                        self.ui(self._draw_timeline_marker, elapsed)
                        time.sleep(0.05)

                    if self.stop_flag.is_set():
                        break

                    self.ui(self.set_status, f"AM cycle {i}/{reps}: OFF")
                    self.am_off()
                ...
                if not self.stop_flag.is_set():
                    ...
                    self.ui(self._draw_timeline_marker, total_time)
                    self.ui(self.set_status, "Done.")
                ...
                self.ui(self.pbar.configure, style="Idle.Horizontal.TProgressbar")
                self.ui(messagebox.showerror, "Run Error", str(e))
