...
class RigolBurstGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Rigol Burst Controller")
    ...
    def build_ui(self):
        ...
        ttk.Entry(main, textvariable=self.visa_var, width=48).grid(row=0, column=1, columnspan=5, sticky="we")
        ...
        ttk.Label(main, text="Repetitions").grid(row=3, column=2, sticky="w")
        ttk.Entry(main, textvariable=self.reps_var, width=10).grid(row=3, column=3, sticky="w")

        # Move Interval next to Duration
        ...
        self.pbar = ttk.Progressbar(self.bar_container, variable=self.progress_var, maximum=100.0, length=360)
        self.pbar.place(x=0, y=0, width=360, height=18)

        # Overlay canvas: EXACT same position/size as progressbar
        ...
        ttk.Label(main, textvariable=self.status_var, foreground="blue").grid(row=6, column=0, columnspan=6, sticky="w", pady=(6, 0))

        self.err_var = tk.StringVar(value="")
        ttk.Label(main, textvariable=self.err_var, foreground="brown").grid(row=7, column=0, columnspan=6, sticky="w")

    # ---------------- Helpers ----------------
    def write(self, cmd): self.instr.write(cmd)
    def query(self, cmd): return self.instr.query(cmd).strip()
    ...
    def _draw_timeline_static(self, reps: int, dur: float, interval: float):
        ...
        self._run_dur = dur
        self._run_int = interval
        self._run_reps = reps

        if total <= 0:
            return

        # Transparent-ish look: we draw only subtle overlays + ticks
        # Burst shading:
        for i in range(reps):
            seg = dur + interval
            bx0 = (i * seg) / total * w
            ...
            # edge at end of each (burst+interval) segment
            ex = ((i + 1) * seg) / total * w
    ...
    def connect(self):
        try:
            ...
            self.instr = self.rm.open_resource(self.visa_var.get().strip())
    ...
    def run_sequence(self):
        ...
        # Draw overlay exactly on the progress bar
        ...
        def worker():
            ...
            try:
                ...
                if self.instr:
                    ...
                    self.idn_btn.config(state="normal")
                self.stop_btn.config(state="disabled")
                self.connect_btn.config(state="disabled" if self.instr else "normal")
